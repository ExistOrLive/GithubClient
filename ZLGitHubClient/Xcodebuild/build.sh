...
xcodebuild archive -workspace ZLGitHubClient.xcworkspace -scheme ZLGitHubClient -archivePath $archivePath
echo "end archive ....................."
...
then
    echo "adhoc build" 
    xcodebuild -exportArchive -allowProvisioningUpdates true -archivePath $archivePath -exportPath $exportDirPath -exportOptionsPlist "Xcodebuild/adhoc.plist"
elif [ $1 = debug ]
then 
    echo "testflight build"
    xcodebuild -exportArchive -allowProvisioningUpdates true -archivePath $archivePath -exportPath $exportDirPath -exportOptionsPlist "Xcodebuild/debug.plist"
elif [ $1 = release ]
then 
    echo "release build"
    xcodebuild -exportArchive -allowProvisioningUpdates true -archivePath $archivePath -exportPath $exportDirPath -exportOptionsPlist "Xcodebuild/debug.plist"
