...
@objcMembers class ZLRepositoryTableViewCellData: ZLGithubItemTableViewCellData {
    ...
    func getOwnerAvatarFromServer() {
        ZLServiceManager.sharedInstance.userServiceModel?.getUserAvatar(withLoginName: self.data.owner?.loginName ?? "", serialNumber: NSString.generateSerialNumber()) { [weak self](resultModel) in
            if let self = self,
