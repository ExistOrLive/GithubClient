...
class ZLRepoWorkflowsViewModel: ZLBaseViewModel {
    ...
    func loadMoreData() {
        ...
        ZLRepoServiceModel.shared().getRepoWorkflows(withFullName: repoFullName ?? "", per_page: 10, page: self.currentPage + 1, serialNumber: NSString.generateSerialNumber()) { (result : ZLOperationResultModel) in
    ...
    func loadNewData() {
        ...
        weak var weakSelf = self
        ZLRepoServiceModel.shared().getRepoWorkflows(withFullName: repoFullName ?? "", per_page: 10, page: 1, serialNumber: NSString.generateSerialNumber()) { (result : ZLOperationResultModel) in
