...
class ZLRepoIssuesController: ZMTableViewController {
...
// MARK: - Action
extension ZLRepoIssuesController {
...
extension ZLRepoIssuesController {
    ...
    func loadData(isLoadNew: Bool) {
        ...
        ZLRepoServiceShared()?
            .getRepositoryIssues(withFullName: repoFullName ?? "",
        ...
        { [weak self](resultModel: ZLOperationResultModel) in
            ...
            if resultModel.result,
               let data = resultModel.data as? [ZLGithubIssueModel] {
                
                let cellDatas = data.map {
                    ZLIssueTableViewCellData(issueModel: $0) 
                }
